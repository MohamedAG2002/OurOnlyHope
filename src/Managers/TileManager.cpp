...
TileManager::TileManager(const std::string&& img)
{
  ...
  m_tiles.emplace_back(std::make_unique<Tile>("West_Wall", // ID 
                                              Vector2{-(TILE_SIZE - 27), TILE_SIZE * 8.5f}, // Position
                                              Vector2{TILE_SIZE, screenSize.y * 2.5f})); // Size

  m_tiles.emplace_back(std::make_unique<Tile>("East_Wall", 
                                              Vector2{screenSize.x + (TILE_SIZE - 22), TILE_SIZE * 8.5f}, 
                                              Vector2{TILE_SIZE, screenSize.y * 2.5f}));

  m_tiles.emplace_back(std::make_unique<Tile>("North_Wall", 
                                              Vector2{TILE_SIZE * 10, -(TILE_SIZE - 27)}, 
                                              Vector2{screenSize.x * 3.0f, TILE_SIZE}));

  m_tiles.emplace_back(std::make_unique<Tile>("South_Wall", 
                                              Vector2{TILE_SIZE * 10, screenSize.y + (TILE_SIZE - 27)}, 
                                              Vector2{screenSize.x * 3.0f, TILE_SIZE}));
}
